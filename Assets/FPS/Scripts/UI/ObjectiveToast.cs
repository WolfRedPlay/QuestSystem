...
namespace Unity.FPS.UI
{
    public class ObjectiveToast : MonoBehaviour
    {
        ...
        //----NEW-----
        [Tooltip("Prefab for objective hint text")]
        ...
        public void Initialize(string titleText, List<Objective> objectives, float delay)
        {
            ...
            {
                GameObject newHintObj = Instantiate(HintPrefab, SubTitleRect);

                Hint newHint = newHintObj.GetComponent<Hint>();

                newHint.SetObjective(obj);
        ...
        void Update()
        {
            float timeSinceFadeStarted = Time.unscaledTime - m_StartFadeTime;

            if (hints.Count > 0)
            {
                SubTitleRect.gameObject.SetActive(true);
            }
